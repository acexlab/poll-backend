...
namespace pollbackend.Services
{
    public class VoteService : IVoteService
    {
        ...
        public async Task<PollResultDto?> GetResultsAsync(long pollId)
        {
            ...
            var participants = votes.Select(v => new ParticipantDto
            {
                VoteId = v.Id,
                Username = v.User?.Username ?? "Unknown User",
                VotedOptionText = v.PollOption?.OptionText ?? "Unknown Option",
                VotedAt = v.VotedAt
            }).ToList();
        ...
        public async Task<bool> DeleteVoteAsync(long id)
        {
        ...
        public async Task<bool> AlterVoteAsync(long voteId, long newOptionId)
        {
            ...
            // Verify the new option belongs to the poll
            if (poll.Options.All(o => o.Id != newOptionId))
            {
                return false;
            }
